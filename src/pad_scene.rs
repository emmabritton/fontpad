...
use crate::{SceneName, SceneResult, Settings, settings};
...
impl PadScene {
    fn copy(&mut self) {
        ...
        self.clipboard.set_contents(output.clone()).expect(&format!("Error copying: {output}"));
    ...
    fn paste(&mut self) {
        match self.clipboard.get_contents() {
            Ok(contents) => {
                self.pad_view.paste_str(contents.trim().trim_end_matches(','));
...
impl Scene<SceneResult, SceneName> for PadScene {
    fn render(&self, graphics: &mut Graphics, mouse: &MouseData, _: &[KeyCode]) {
        ...
        graphics.draw_text(&format!("{}", self.pad_view.size.0), TextPos::px(WIDTH_POS), (WHITE, Normal));
        graphics.draw_text(&format!("{}", self.pad_view.size.1), TextPos::px(HEIGHT_POS), (WHITE, Normal));
    }
    ...
    fn on_key_up(&mut self, key: KeyCode, _: &MouseData, held: &[KeyCode]) {
        ...
        match key {
            ...
            KeyCode::KeyC => if modifier_pressed { self.copy() },
            KeyCode::KeyV => if modifier_pressed { self.paste() },
            _ => {}
        }
    }

    fn on_mouse_click(&mut self, down_at: Coord, mouse: &MouseData, mouse_button: MouseButton, _: &[KeyCode]) {
    ...
    fn update(&mut self, timing: &Timing, mouse: &MouseData, _: &[KeyCode]) -> SceneUpdateResult<SceneResult, SceneName> {
        if let Some(_) = mouse.is_down(MouseButton::Left) {
            if self.next_update.update(timing) {
